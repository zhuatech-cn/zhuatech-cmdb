...
import cn.zhuatech.cmdb.service.CmdbService;
...
public class CmdbController {
    private final CmdbService service; public CmdbController(CmdbService service){this.service=service;}
    @GetMapping("/public/about") ApiResponse<Map<String,Object>> about(){return ApiResponse.ok(service.about());}
    @GetMapping("/catalog") ApiResponse<CmdbService.CatalogView> catalog(){return ApiResponse.ok(service.catalog());}
    @GetMapping("/dashboard") ApiResponse<CmdbService.Dashboard> dashboard(){return ApiResponse.ok(service.dashboard());}
    ...
    @PostMapping("/records/{id}/comments") ApiResponse<BusinessRecord> comment(@PathVariable Long id,@Valid @RequestBody CmdbService.CommentRequest request){return ApiResponse.ok(service.comment(id,request));}
    @GetMapping("/sla-summary") ApiResponse<CmdbService.SlaSummary> sla(){return ApiResponse.ok(service.slaSummary());}
    @PostMapping("/records") ApiResponse<BusinessRecord> create(@Valid @RequestBody CmdbService.RecordRequest request){return ApiResponse.ok(service.create(request));}
    @PutMapping("/records/{id}") ApiResponse<BusinessRecord> update(@PathVariable Long id,@Valid @RequestBody CmdbService.RecordRequest request){return ApiResponse.ok(service.update(id,request));}
    @PostMapping("/records/{id}/actions") ApiResponse<BusinessRecord> action(@PathVariable Long id,@Valid @RequestBody CmdbService.ActionRequest request){return ApiResponse.ok(service.action(id,request));}
